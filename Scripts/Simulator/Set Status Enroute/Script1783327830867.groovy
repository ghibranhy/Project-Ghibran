...
// 2. TANGKAP VARIABEL PAYMENT (ECV = 1)
String epayValue = "1" // Default ke ECV
if (binding.hasVariable('epay_value') && epay_value != null) {
	epayValue = epay_value.toString()
}
String stateValue = "1" 

KeywordUtil.logInfo("========== [SIMULATOR] SET STATUS ENROUTE ==========")
KeywordUtil.logInfo("📱 Order ID : ${orderIdCustom} | 💳 E-Pay Flag : ${epayValue}")

// 3. Setup Request Object
RequestObject ro = new RequestObject('SOAP_State_1_Dynamic')
...
// 4. Rakit XML (VARIABEL epayValue DIMASUKKAN KE SINI)
String innerOrderInfo = "&lt;set&gt;&lt;orderInfo&gt;" +
	...
	"&lt;epayFlag&gt;" + epayValue + "&lt;/epayFlag&gt;" +     // 🔥 INI KUNCINYA
	"&lt;finalEpay&gt;" + epayValue + "&lt;/finalEpay&gt;" + // 🔥 INI KUNCINYA
...
String xmlPayload = '<x:Envelope xmlns:x="http://schemas.xmlsoap.org/soap/envelope/" xmlns:sil="http://siliconstraits.com">' +
	...
	'<x:Body>' +
	  '<sil:sendOrderInfo>' +
	  '<sil:orderInfo>' + innerOrderInfo + '</sil:orderInfo>' + // 🗑️ InsufficientBalance DIBUANG
	  '</sil:sendOrderInfo>' +
	'</x:Body>' +
'</x:Envelope>'
